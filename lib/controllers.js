const async          = require('async')
    ...
    , debug          = require('debug')('splinky-controllers')
    , debugFilter    = require('debug')('splinky-filters')
...
function createInvoker (viewHandler, filters, ctx, handler) {
  return function (req, res, params) {
    ...
    debug('request', requestContext.request.method, requestContext.request.url)
...
function initController (key, callback) {
  var meta    = this.externalSplink.meta(key)
    ...
    , methods = meta.method ? Array.isArray(meta.method) ? meta.method : [ meta.method ] : [ 'get' ]
    ...
    this.filterManager(meta.filters, function (err, filters) {
      ...
      routes.forEach(function (route) {
        methods.forEach(function (method) {
          ...
          debug(
              'attached controller [%s] for %s requests to route [%s]'
            , key
            , method.toUpperCase()
            , route
          )
